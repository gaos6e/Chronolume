...
- 新增 `gpt-6-sol` 与 `gpt-6-luna` 内置价格、显示名称和模型排序；标准短上下文每百万 Token 输入/缓存读取/缓存写入/输出参考价分别为 Sol $2/$0.2/$2.5/$10、Luna $0.1/$0.01/$0.125/$0.5。
- 更新内置价格修订号，已有分析库会在启动后自动重算历史记录。

## 2.1.5 — 2026-09-05

- 新增 `gpt-6-astra` 内置价格与 `GPT-6 Astra` 显示名称；标准短上下文每百万 Token 输入/缓存读取/缓存写入/输出参考价分别为 $10/$1/$12.5/$50。
- 更新内置价格修订号，启动后自动重算历史未定价记录，无需清空分析库。
- 同步应用版本显示并补充 Astra 计价、入库与排序测试。
- 当前仍使用标准短上下文参考价估算，未自动区分长上下文与 Fast 等服务档位。

## 2.1.4 — 2026-07-22

- 修复分叉会话在嵌入父历史后恢复子会话记录时被提前跳过的问题；现在只跳过复制的父段，并在子会话元数据恢复后继续统计。
- 解析器升级时重放全部旧 JSONL 来源，重建会话模型段、日汇总和总计，消除历史事件已正确但派生摘要仍重复的残留数据。

## 2.1.3 — 2026-07-22

- 修复分叉会话 JSONL 中嵌入父会话历史被重复计入的问题；解析器以当前分叉会话的 `id` 识别来源，并在检测到嵌入历史后跳过其余副本。
- 升级时会重放受影响来源并重建原父会话的模型、日汇总和 Token 摘要，纠正已经出现的异常超大用量。

## 2.1.2 — 2026-07-22

- 修复同一会话跨多个 JSONL 来源时的模型 Token 串账：会话模型摘要改由逐条用量事件重建，避免本地事件序号重叠和模型切换顺序导致错误归因。
- 解析器升级后自动重放既有来源；会话详情按模型汇总展示准确的 Token 与成本，并保留无法归因的 `unknown` 用量。
